...
| **Shared-quote** | **Neither** side is vetted (alt→alt, e.g. BONK→WIF) | Two hops through a mint both tokens trade against (usually WSOL): sell input → shared mint, then shared mint → output. Discovery lives in ix-builder `shared-quote-bridge.js`; protocol often shows as `SHARED_QUOTE_BRIDGE`. Hop preference: **ALT-covered CLMM/DLMM first** (known accounts in the Vybe route ALTs), then Pumpfun / Launch Lab → Pumpswap → DBC → AMMv4 → DAMM2 → CPMM → **uncovered CLMM / DLMM last**. |
