...
| **Shared-quote** | **Neither** side is vetted (alt→alt, e.g. BONK→WIF) | Two hops through a mint both tokens trade against (usually WSOL): sell input → shared mint, then shared mint → output. Discovery lives in ix-builder `shared-quote-bridge.js`; protocol often shows as `SHARED_QUOTE_BRIDGE`. Hop preference: Pumpfun / Launch Lab → DBC → AMMv4 → DAMM2 → CPMM → Pumpswap → **CLMM / DLMM last**. When picking among CLMM/DLMM pools only, prefer **ALT-covered** (known-account) pools. |
