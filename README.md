...
| **Shared-quote** | **Neither** side is vetted (alt→alt, e.g. BONK→WIF) | Two hops through a mint both tokens trade against (usually WSOL): sell input → shared mint, then shared mint → output. Discovery lives in ix-builder `shared-quote-bridge.js`; protocol often shows as `SHARED_QUOTE_BRIDGE`. Hop preference: **ALT-covered** → Pumpfun / Launch Lab → DBC → **DAMM2** → **AMMv4 (USD liq > $1k)** → CPMM → Pumpswap → CLMM / DLMM → thin AMMv4. If enumerated AMMv4 output is **>2% worse** than CLMM/DLMM, pick the **smallest tx** among the top 3 of those two families. |
